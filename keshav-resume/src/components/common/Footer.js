import React from "react";
import linkedinpic from "../../assets/images/linkedin.png";
import leetcodepic from "../../assets/images/leetcode.png";
import instagrampic from "../../assets/images/instagram.png";
export const Footer = () => {
    return (
        <div className="footer pb-5 pt-2">
            <footer className="d-flex flex-wrap justify-content-between align-items-center">
                <div className="col-md-4 d-flex align-items-center">
                    <a href="/" className="mb-3 me-2 mb-md-0 text-muted text-decoration-none lh-1">

                    </a>
                    <span className="mb-3 mb-md-0 text-muted" style={{ 'fontWeight': 'bold' }}>© 2023 KeshavaResume</span>
                </div>
                <ul class="nav col-md-4 justify-content-end list-unstyled d-flex">
                    <li className="ms-3 p-2"><a className="text-muted" href="#"></a> <img src={linkedinpic} /></li>
                    <li className="ms-3 p-2"><a className="text-muted" href="#"></a>  <img src={leetcodepic} style={{ 'height': '47px', 'width': '47px' }} /></li>
                    <li className="ms-3 p-2"><a className="text-muted" href="#"></a>  <img src={instagrampic} /></li>
                </ul>
            </footer>
        </div>

    )
}
export default Footer;